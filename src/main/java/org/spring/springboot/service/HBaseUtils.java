...
public class HBaseUtils {
     ...
     * @return map
     */
    public List<Map<String, String>> getAllVideoTable(String tableName) {
        List<Map<String, String>> list = new ArrayList<>();
        try {
            Table table = hbaseAdmin.getConnection().getTable(TableName.valueOf(tableName));
            Scan scan = new Scan();
            ...
            for(Result result : resultScanner) {
                ...
                for (Cell cell : result.listCells()) {
                    ...
                    String qualifier = Bytes.toString(cell.getQualifierArray(),
                            cell.getQualifierOffset(), cell.getQualifierLength());
                    //值
                    Double data = Bytes.toDouble(cell.getValueArray(), cell.getValueOffset());
                    map.put(family + ":" + qualifier, String.valueOf(data));
                }
                list.add(map);
            }
            table.close();
     ...
     * @return map
     */
    public List<Map<String, String>> getAllProblemTable(String tableName) {
        List<Map<String, String>> list = new ArrayList<>();
        try {
            Table table = hbaseAdmin.getConnection().getTable(TableName.valueOf(tableName));
            Scan scan = new Scan();
            ...
            for(Result result : resultScanner) {
                ...
                for (Cell cell : result.listCells()) {
                    ...
                    //值
                    if(i % 5 <= 3) {
                        Double data = Bytes.toDouble(cell.getValueArray(), cell.getValueOffset()-2);
                        map.put(family + ":" + qualifier, String.valueOf(data));
                    }
                    else{
                        String data = Bytes.toString(cell.getValueArray(),
        ...
        try {
            ...
            if (result != null && !result.isEmpty()) {
                ...
                data = Bytes.toString(cell.getValueArray(), cell.getValueOffset(),
                        cell.getValueLength());
//                data = new String(result.getValue(columnFamily.getBytes(),columnQualifier==null?null:columnQualifier.getBytes()));
            }
            table.close();
        } catch (IOException e) {
            e.printStackTrace();
     ...
     * @return map
     */
    public Double getDoubleData(String tableName, String rowKey, String columnFamily,
                                String columnQualifier) {
        Double data = -1.0 ;
        try {
            Table table = hbaseAdmin.getConnection().getTable(TableName.valueOf(tableName));
            ...
            if (result != null && !result.isEmpty()) {
                Cell cell = result.listCells().get(0);
                data = Bytes.toDouble(cell.getValueArray(), cell.getValueOffset());
            }
            table.close();
        } catch (IOException e) {
            e.printStackTrace();
